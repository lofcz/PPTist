...
const insertVideo = () => {
  if (!videoSrc.value) return message.error('Please enter the correct video address')
  emit('insertVideo', videoSrc.value)
}

const insertAudio = () => {
  if (!audioSrc.value) return message.error('Please enter the correct audio address')
