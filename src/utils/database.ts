...
// When the application is closed (closing or refreshing the browser), its database ID will be recorded in localStorage, indicating that the database pointed to by this ID is invalid.
// When the application initializes, check all current databases and delete those marked as invalid.
// Additionally, databases older than 12 hours from the initialization time will also be deleted (this is to prevent databases that were not correctly deleted due to unexpected circumstances).
